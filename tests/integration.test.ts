...
describe('Integration Tests with Mocks', () => {
    ...
    it('should auto-detect formats from mock files', () => {
        const avanzaCsv=fs.readFileSync(path.join(mocksDir, 'avanza.csv'), 'utf8');
        ...
        const nordnetCsv=fs.readFileSync(path.join(mocksDir, 'nordnet.csv'), 'utf8');
        const nordnetRow=Papa.parse(nordnetCsv, { header: true }).data[0] as any;
        expect(parseTransaction(nordnetRow, 'Auto')?.originalSource).toBe('Nordnet');
    });
});
